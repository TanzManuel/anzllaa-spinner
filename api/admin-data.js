...
export default async function handler(req,res){if(!requireAdmin(req,res))return;const [available,used,history]=await Promise.all([scan('spin:available'),scan('spin:used'),redis.lrange('spin:history',0,199)]);return json(res,200,{ok:true,available,used,history:history.map(x=>{try{return typeof x==='string'?JSON.parse(x):x}catch{return null}}).filter(Boolean)})}
